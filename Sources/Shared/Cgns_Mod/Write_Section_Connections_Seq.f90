...
!-----------------------------------[Locals]-----------------------------------!
  ...
  integer              :: error
  integer              :: n_nodes, loc, c, n, cell, dir, cnt, bc, i
...
!==============================================================================!
  ...
  base_id    = base
  block_id   = block
  sect_name  = trim(cgns_base(base) % block(block) % section(sect) % name)
  ...
  if ( last_cell .ne. 0 ) then
    ...
    do c = 1, grid % n_cells
      if (n_nodes .eq. 8) then ! hex
        ...
        i = i + 1
      elseif (n_nodes .eq. 6) then ! wedge
        cell_n (1:6, i) = grid % cells_n(1:6, c)
        i = i + 1
      elseif (n_nodes .eq. 5) then ! pyramid
        ...
        i = i + 1
      elseif (n_nodes .eq. 4) then ! tetra
