!======================================================================*
      PROGRAM interpolate_on_new_mesh
!----------------------------------------------------------------------*
...
!----------------------------------------------------------------------!
  ...
  write(*,*)' Enter name of old case without ext. : '
  read(*,*) name 
  write(*,*)' Enter name of new case without ext. : '
  read(*,*) namSav 
  write(*,*)' Enter number of subdomains in new case: '
  read(*,*) ND 
  write(*,*)' Enter 1 if T will be interpolated and 0 if not: '
  read(*,*) IND
        ...
        do k = 1, NCold
          if(Xold(k) <= Xmax.and.Xold(k) >= Xmin) then
            if(Yold(k) <= Ymax.and.Yold(k) >= Ymin) then
              if(Zold(k) <= Zmax.and.Zold(k) >= Zmin) then
                NN = NN + 1
                write(9,'(36E14.6)') Xold(k), Yold(k), Zold(k), &
        ...
        do k = 1, NCold
          if(Xold(k) <= Xmax.and.Xold(k) >= Xmin) then
            if(Yold(k) <= Ymax.and.Yold(k) >= Ymin) then
              if(Zold(k) <= Zmax.and.Zold(k) >= Zmin) then
                NN = NN + 1
                write(9,'(29E14.6)') Xold(k), Yold(k), Zold(k), &
                                    Uold(k), Uoold(k), UCold(k), UCoold(k), UDoold(k), UXold(k), UXoold(k), &
                                    Vold(k), Voold(k), VCold(k), VCoold(k), VDoold(k), VXold(k), VXoold(k), &
                                    Wold(k), Woold(k), WCold(k), WCoold(k), WDoold(k), WXold(k), WXoold(k), &
                                    Pold(k), PPold(k), Pxold(k), Pyold(k), Pzold(k)
