...
!--------------------------------------------------------------------------------------------!
!     The formulation that resulted from the work with Maarten van Rijwijk
!--------------------------------------------------------------------------------------------!
      Ret = Kin % n(c)*Kin % n(c)/(VISc*Eps % n(c))
      Fmu = exp(-3.8/(1.0 + 0.02*Ret)**2.0) 
      yStar = (VISc * Eps % n(c))**0.25 * WallDs(c)/VISc
      PHI4x(c) = (1.0 + 0.5*exp(-(Ret/15.0)*(Ret/15.0)))**3.0
      VISt(c) = PHI4x(c) * Fmu * Cmu * DENc(material(c)) * Kin%n(c) * Kin%n(c) / Eps % n(c)
    end do
  end if
