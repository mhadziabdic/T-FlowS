!======================================================================!
  subroutine UserCutLines_X_dir(grid) 
!----------------------------------------------------------------------!
...
!---- write the probe coordinates out
    ...
    call GraPhi(U % mean, 1, PHI3x,.TRUE.)
...
!+++++++++++++++++++++++++++++!
  do k = 1, N_hor
    do i = 1, Nprob-1
      do c=1,NC
        Rad_2 = xc(c) 
        if(yc(c) < r1_p(k) .and. yc(c) > r2_p(k)) then
          if(Rad_2 > z_p(i) .and. Rad_2 < z_p(i+1)) then
            if(SIMULA==LES) then      
              ...
              var_2(i) = var_2(i) -(uu%mean(c)+(VAR10x(c)-U % mean(c)*U%mean(c))-&
                                      vv%mean(c)-(VAR10y(c)-V%mean(c)*V%mean(c)))*PHI3x(c)
...
!---- average over all processors
    ...
    do i = 1, Nprob
      if(Ncount(i) /= 0) then
        ...
        write(3,'(19E15.7, I7)') (z_p(i)+z_p(i+1))/2.0, Ump(i), Vmp(i), Wmp(i) , uup(i), vvp(i), & 
                                wwp(i), uvp(i), uwp(i), vwp(i), &
                                uum(i), vvm(i), wwm(i), uvm(i), uwm(i), vwm(i), var_1(i), &
                                0.5*(uum(i)+vvm(i)+wwm(i)), var_2(i), Ncount(i)
