!======================================================================!
  subroutine UserCutLines_channel(grid, y) 
!----------------------------------------------------------------------!
...
!            fp(i)    = fp(i)  + grid % vol(c)**0.333/(VISc**3/Eps % n(c))**0.25 !f22%n(c)   !tt % n(c) ! - VISt(c)*PHIz(c)
!            v2p(i)   = v2p(i) + v_2 % n(c) 
            fp(i)    = fp(i)  + f22 % n(c) 
            uup(i)   = uup(i) + 2.0*VISt(c)*Ux(c) 
            vvp(i)   = vvp(i) + 2.0*VISt(c)*Vy(c)
...
!              print *, wt % n(c)
            ...
            Epsp(i)  = Epsp(i) + Eps % n(c) 
            v2p(i)   = v2p(i) + v_2 % n(c) 
            fp(i)    = fp(i) +  grid % vol(c)**0.333/(VISc**3/Eps % n(c))**0.25 !f22 % n(c) 
            ...
            Kinp(i)  = Kinp(i) + Kin % n(c) 
            Epsp(i)  = Epsp(i) + Eps % n(c) 
            fp(i)    = fp(i) + 1.5 + 0.4/(1.0 + 3.0*(0.41*WallDs(c)/grid % vol(c)**ONE_THIRD)**0.5 ) 
            if(IsNearWall(c)) then
              Ufric_p(i) = Ufric_p(i) + (VISc * (U % mean(c)**2 + V % mean(c)**2 + W % mean(c)**2)**0.5/WallDs(c))**0.5
            end if 
          end if 
