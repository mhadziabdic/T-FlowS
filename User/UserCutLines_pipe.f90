...
!======================================================================!
    ...
    namRes_plus = name
    namRes_plus(len_trim(name)+1:len_trim(name)+13) = '_res_nonD.dat'
!>>>>>>>>>>>>>>>>>>>>>>!
!     read 1D file     !
!>>>>>>>>>>>>>>>>>>>>>>!
    namCoo = name
...
10  continue
    ...
    if(SIMULA == LES.or.SIMULA == DES_SPA.or.SIMULA == DNS) then
      ...
      do i = 1, Nprob-1
        if(Ncount(i) /= 0) then
          write(3,'(12E15.7)') Wall_p(i), abs(Ump(i)), abs(Vmp(i)), abs(Wmp(i)),   &
    ...
    if(SIMULA == LES.or.SIMULA == DES_SPA.or.SIMULA==DNS) then
      ...
      do i = 1, Nprob-1
        if(Ncount(i) /= 0) then
          ...
          uup(i)/Ufric**2.0, vvp(i)/Ufric**2.0, wwp(i)/Ufric**2.0, uvp(i)/Ufric**2.0, uwp(i)/Ufric**2.0, &
          vwp(i)/Ufric**2.0, var_1(i)/Ufric**2.0, var_2(i)*VISc/Ufric**4.0  
        end if
      end do 
    else if(SIMULA == ZETA) then
      write(3,'(A1,1X,A80)') '#', '1:Xrad+, 2:U+, 3:V+, 4:W+, 5:Kin+, 6:Eps+, 7:zeta+, 8:f+                     ' 
