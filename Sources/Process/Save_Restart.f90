...
!==============================================================================!
  ...
  write(9)        0,      grid % n_boundary_cells,       grid % n_cells,       grid % n_faces,     Ndtt,    Nstat
  ...
  write(9) (U % n(c),   c = -grid % n_boundary_cells,grid % n_cells)
  write(9) (V % n(c),   c = -grid % n_boundary_cells,grid % n_cells)
  write(9) (W % n(c),   c = -grid % n_boundary_cells,grid % n_cells)
  ...
  write(9) (P % n(c),   c = -grid % n_boundary_cells,grid % n_cells)
  write(9) (PP % n(c),  c = -grid % n_boundary_cells,grid % n_cells)

  write(9) (Px(c),   c = -grid % n_boundary_cells,grid % n_cells)
  write(9) (Py(c),   c = -grid % n_boundary_cells,grid % n_cells)
  write(9) (Pz(c),   c = -grid % n_boundary_cells,grid % n_cells)
  ...
  if(HOT == YES) then
    write(9) (T % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (T % q(c),    c = -grid % n_boundary_cells,-1)
  ...
  if(SIMULA==K_EPS.or.SIMULA==ZETA.or.&
    ...
    write(9) (Kin % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (Eps % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (Pk(c),     c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (Uf(c),     c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (Ynd(c),    c = -grid % n_boundary_cells,grid % n_cells) 
    write(9) (VISwall(c),c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (TauWall(c),c= 1,grid % n_cells)
  end if

  if(SIMULA==K_EPS_VV.or.SIMULA==ZETA.or.SIMULA == HYB_ZETA) then
    write(9) (v_2%n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (f22%n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (Tsc(c),    c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (Lsc(c),    c = -grid % n_boundary_cells,grid % n_cells)
  end if 

  if(SIMULA==EBM.or.SIMULA==HJ) then
    write(9) (uu  % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (vv  % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (ww  % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (uv  % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (uw  % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (vw  % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (Pk(c),         c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (Kin % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (VISt(c),       c = -grid % n_boundary_cells,grid % n_cells)
    ...
    if(URANS == YES) then
      write(9) (VAR10x(c),    c = -grid % n_boundary_cells,grid % n_cells)
      write(9) (VAR10y(c),    c = -grid % n_boundary_cells,grid % n_cells)
      write(9) (VAR10z(c),    c = -grid % n_boundary_cells,grid % n_cells)
      write(9) (VAR11x(c),    c = -grid % n_boundary_cells,grid % n_cells)
      write(9) (VAR11y(c),    c = -grid % n_boundary_cells,grid % n_cells)
      write(9) (VAR11z(c),    c = -grid % n_boundary_cells,grid % n_cells)
    end if  
  end if

  if(SIMULA == SPA_ALL.or.SIMULA==DES_SPA) then
    write(9) (VIS % n(c),    c = -grid % n_boundary_cells,grid % n_cells)
    ...
    write(9) (Vort(c),       c = -grid % n_boundary_cells,grid % n_cells)
  end if

  if(SIMULA/=DNS) write(9) (VISt(c), c = -grid % n_boundary_cells,grid % n_cells)

  if(SIMULA==DNS.or.SIMULA==LES.or.SIMULA==URANS.or.&
     SIMULA==HYB_ZETA.or.SIMULA==HYB_PITM.or.SIMULA==DES_SPA) then
    write(9) (U % mean(c),  c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (V % mean(c),  c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (W % mean(c),  c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (uu % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (vv % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (ww % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (uv % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (uw % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (vw % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
    write(9) (P % mean(c),  c = 1, grid % n_cells)

    if(HOT == YES) then
      write(9) (T % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
      write(9) (TT % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
      write(9) (uT % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
      write(9) (vT % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
      write(9) (wT % mean(c), c = -grid % n_boundary_cells,grid % n_cells)
    end if
  end if

  if(SIMULA == LES) then
    write(9) (VISt_mean(c), c = 1, grid % n_cells)
    write(9) (near(c),   c = -grid % n_boundary_cells,grid % n_cells)
