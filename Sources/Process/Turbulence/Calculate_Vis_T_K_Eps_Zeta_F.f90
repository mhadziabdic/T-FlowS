    ...
    if(c2 < 0 .and. Grid_Mod_Bnd_Cond_Type(grid,c2) /= BUFFER) then
      if(Grid_Mod_Bnd_Cond_Type(grid,c2)==WALL .or.  &
        ...
        if(y_pl < 3.0) then
          ...
          vis_wall(c1) = y_plus(c1)*(viscosity/density)/ &
            (y_pl*exp(-1.0*g_blend) + u_plus*exp(-1.0/g_blend) + TINY)
        end if

        if(ROUGH == YES) then
          u_plus = log((grid % wall_dist(c1)+Zo)/Zo)/(kappa + TINY) + TINY
          vis_wall(c1) = min(y_pl*(viscosity/density)*kappa/ &
            LOG((grid % wall_dist(c1)+Zo)/Zo),1.0e+6*(viscosity/density))
       end if
        ...
        if(heat_transfer == YES) then
          ...
          con_wall(c1) = y_pl*(viscosity/density)*capacity/(y_pl*pr_mol* &
