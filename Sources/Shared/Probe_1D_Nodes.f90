...
!==============================================================================!
  ...
  do c=-NbC, NC
    do n = 1, grid % cells_n_nodes(c)
      ...
      do p=1,n_prob
          if(answer == 'X') then
            if( Approx(grid % nodes(grid % cells_n(n,c)) % x, n_p(p)) ) go to 1
          else if(answer == 'Y') then
            if( Approx(grid % nodes(grid % cells_n(n,c)) % y, n_p(p)) ) go to 1
          else if(answer == 'Z') then
            if( Approx(grid % nodes(grid % cells_n(n,c)) % z, n_p(p)) ) go to 1
          else if(answer == 'RX') then
            if( Approx( (grid % nodes(grid % cells_n(n,c)) % z**2 +   &
                         grid % nodes(grid % cells_n(n,c)) % y**2)**.5, n_p(p)) ) go to 1
          else if(answer == 'RY') then
            if( Approx( (grid % nodes(grid % cells_n(n,c)) % x**2 +   &
                         grid % nodes(grid % cells_n(n,c)) % z**2)**.5, n_p(p)) ) go to 1
      ...
      n_prob = n_prob+1
      if(answer=='X') n_p(n_prob) = grid % nodes(grid % cells_n(n,c)) % x
      if(answer=='Y') n_p(n_prob) = grid % nodes(grid % cells_n(n,c)) % y
      if(answer=='Z') n_p(n_prob) = grid % nodes(grid % cells_n(n,c)) % z

      if(answer=='RX') n_p(n_prob) =                                           &
                         (grid % nodes(grid % cells_n(n,c)) % z**2.0 +      &
                          grid % nodes(grid % cells_n(n,c)) % y**2.0)**0.5
      if(answer=='RY') n_p(n_prob) =                                           &
                         (grid % nodes(grid % cells_n(n,c)) % x**2.0 +      &
                          grid % nodes(grid % cells_n(n,c)) % z**2.0)**0.5
