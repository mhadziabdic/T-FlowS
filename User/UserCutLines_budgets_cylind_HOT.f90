...
!--------------------------------[CVS]---------------------------------!
!  $Id: UserCutLines_budgets_cylind_HOT.f90,v 1.2 2017/08/31 22:42:35 mhadziabdic Exp $  
!  $Source: /home/mhadziabdic/Dropbox/cvsroot/T-FlowS-CVS/User/UserCutLines_budgets_cylind_HOT.f90,v $  
!======================================================================!
...
!      Ufric = (VISc*abs(Wmp(1))/(Rad_mp(1)))**0.5
      if( Ncount(i) > 2) then
        ...
        -Ptet(i)*NF, Ctt(i)*NF*vol1, Diffv_tt(i)*NF*vol1, -Difft_tt(i)*NF, Diss_tt(i)*NF, &
        Ncount(i)
      end if
    end do
    close(4)
