...
!--------------------------------[CVS]---------------------------------!
!  $Id: UserCutLines_Horiz_RANS.f90,v 1.2 2017/08/31 22:42:35 mhadziabdic Exp $  
!  $Source: /home/mhadziabdic/Dropbox/cvsroot/T-FlowS-CVS/User/UserCutLines_Horiz_RANS.f90,v $  
!======================================================================!
...
!     read 1D file     !
!>>>>>>>>>>>>>>>>>>>>>>!
    if(this < 2) write(6, *) '# Now reading the file: rad_coordinate.dat ' 
      ...
      do c=1,NC
        ...
        if(xc(c) < r1_p(k) .and. xc(c) > r2_p(k)) then
          if(Rad_2 > z_p(i) .and. Rad_2 < z_p(i+1)) then
            ...
            wwp(i)   = wwp(i) + v_2 % n(c)
            uvp(i)   = uvp(i) + f22%n(c)
            uwp(i)   = uwp(i) + VISt(c)/VISc

!            var_1(i) = var_1(i) + P % n(c)
...
!---- average over all processors
    ...
    do i = 1, Nprob
      if(Ncount(i) /= 0) then
        ...
        write(3,'(9E15.7, I7)') (z_p(i)+z_p(i+1))/2.0, Ump(i), Vmp(i), Wmp(i) , uup(i), vvp(i), wwp(i), uvp(i), uwp(i), Ncount(i)
