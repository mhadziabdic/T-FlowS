  ...
  do s = 1, grid % n_faces
    ...
    if(c2 < 0 .and. Grid_Mod_Bnd_Cond_Type(grid,c2) /= BUFFER) then
      if(Grid_Mod_Bnd_Cond_Type(grid,c2)==WALL .or.  &
        ...
        if(ROUGH == YES) then
          y_plus(c1) = (grid % wall_dist(c1)+Zo)*u_tau(c1)/(viscosity/density)
        else if(ROUGH == NO) then
          y_plus(c1) = grid % wall_dist(c1)*u_tau(c1)/(viscosity/density) 
        end if
        ...
        if(y_pl< 3.0) then
          vis_wall(c1) = vis_t(c1) + (viscosity/density)  
        else
          vis_wall(c1) = y_plus(c1)*(viscosity/density)/(y_pl*exp(-1.0*g_blend) &
                        +u_plus*exp(-1.0/g_blend) + TINY)
        end if

        if(ROUGH == YES) then
          u_plus = log((grid % wall_dist(c1)+Zo)/Zo)/(kappa + TINY) + TINY
          vis_wall(c1) = min(y_pl*(viscosity/density)*kappa/LOG((grid % wall_dist(c1)+Zo)/Zo),1.0e+6*(viscosity/density))
       end if

        if(heat_transfer == YES) then
          pr_turb = 1.0 / ( 0.5882 + 0.228*vis_t(c1)/(viscosity/density)   &
                 - 0.0441 * (vis_t(c1)/(viscosity/density))**2.0  &
                 * (1.0 - exp(-5.165*(viscosity/density)/(vis_t(c1)+tiny))) )
          pr_mol = (viscosity/density) * capacity / conductivity
          beta = 9.24 * ((pr_mol/pr_turb)**0.75 - 1.0) * (1.0 + 0.28 * exp(-0.007*pr_mol/pr_turb))
          ebf = 0.01 * (pr_mol*y_pl)**4.0 / (1.0 + 5.0 * pr_mol**3.0 * y_pl) + TINY
          con_wall(c1) = y_pl*(viscosity/density)*capacity/(y_pl*pr_mol*exp(-1.0 * ebf) &
